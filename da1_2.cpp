// Create a circular doubly linked list to store the following details of students – name and state. Start from 
// the first node and traverse the list both in the forward and backward direction until a student from the same
// state is found and delete that node. Delete only one node that comes first in the forward or backward 
// direction. Now move to the next node from the currently deleted node. Continue traversing and deleting in a
// similar way until all the duplicates are removed. Print the final list.

//either perform back or front insertion
